...
def selftraining(sess, classifier, label_data, unlabel_data, test_data, FLAGS):
    ...
    with tf.name_scope('train'):
        ...
        #optimizer = classifier.training_op(loss, tf.trainable_variables(), FLAGS.grad_clip, 20, FLAGS.learning_rate)
        optimizer = classifier.get_training_op(loss, classifier.init_lr)
            ...
            for samples, in train_it:
                ...
                if best_acc < test_acc:
                    ...
                    for samples, in unlabel_it:
                        ...
                        if len(_prob) != len(samples):
                            print('WTF')

                    top_k_id = np.argsort(_probs)[::-1][:NUM_SELECT]
                    print([_probs[idx] for idx in top_k_id])
...
def main(_):
    ...
    data_dir = 'classifier/data/rest/memnet/'
...
#            self.target_embedding = tf.get_variable('target_embedding', [len(target2idx), embedding_dim], initializer=tf.constant_initializer(target_embedding))
        ...
        word_embedding = np.vstack((word_embedding, np.zeros([1, FLAGS.embedding_dim])))

        classifier = MEMClassifier(nwords=len(word2idx)+1,
            word2idx = word2idx,
