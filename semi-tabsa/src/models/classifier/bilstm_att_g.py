...
class BilstmAttGClassifier(BaseModel):
    def __init__(self, word2idx, embedding_dim, n_hidden, learning_rate, n_class, max_sentence_len, l2_reg, embedding, grad_clip):
        ...
        self.W_att = tf.get_variable(
            name='W_att',
            shape=[2*self.n_hidden + self.embedding_dim, self.n_hidden],
        ...
        self.W1 = tf.get_variable(
            name='W1',
            shape=[2*self.n_hidden, 2*self.n_hidden],
            initializer=tf.random_uniform_initializer(-0.01, 0.01),
            regularizer=tf.contrib.layers.l2_regularizer(self.l2_reg)
        )
        ...
        self.U1 = tf.get_variable(
            name='U1',
            shape=[self.embedding_dim, 2*self.n_hidden],
            initializer=tf.random_uniform_initializer(-0.01, 0.01),
            regularizer=tf.contrib.layers.l2_regularizer(self.l2_reg)
        )
        self.U2 = tf.get_variable(
            name='U2',
            shape=[self.embedding_dim, 2*self.n_hidden],
            initializer=tf.random_uniform_initializer(-0.01, 0.01),
            regularizer=tf.contrib.layers.l2_regularizer(self.l2_reg)
        )
        self.U3 = tf.get_variable(
            name='U3',
            shape=[self.embedding_dim, 2*self.n_hidden],
            initializer=tf.random_uniform_initializer(-0.01, 0.01),
            regularizer=tf.contrib.layers.l2_regularizer(self.l2_reg)
    ...
    def dynamic_rnn(self, inputs, length, max_len, scope_name, is_reuse=False, out_type='all'):
        with tf.variable_scope("bidirection_lstm", reuse=is_reuse) as bilstm_scope:
    ...
    def AT(self, inputs, target, sen_len, is_reuse=False, type_=''):

        batch_size = tf.shape(inputs)[0]
        target = tf.reshape(target, [-1, 1, self.embedding_dim])
        #target = tf.tile(target, [1, self.max_sentence_len, 1])
        target = tf.ones([batch_size, self.max_sentence_len, self.embedding_dim], dtype=tf.float32) * target
        #in_t = tf.concat([inputs, target], 2)
        #in_t = tf.nn.dropout(in_t, keep_prob=1.0)
        #cell = tf.nn.rnn_cell.LSTMCell
        #hiddens = self.bi_dynamic_rnn(cell, in_t, self.sen_len, self.max_sentence_len, 'AT', 'all')
        #hiddens = self.bi_dynamic_rnn(inputs, sen_len, self.max_sentence_len, 'AT', is_reuse, 'all')
        hiddens = self.bi_dynamic_rnn(inputs, sen_len, self.max_sentence_len, 'AT', is_reuse, 'all')

        h_t = tf.reshape(tf.concat([hiddens, target], 2), [-1, 2*self.n_hidden + self.embedding_dim])
        ...
        att_vecs = tf.reshape(tf.matmul(alpha, hiddens), [-1,  2*self.n_hidden])
    ...
    def bilstm_att_g(self, target, context, context_left, context_right, sen_len, sen_len_left, sen_len_right, keep_rate):
        """
        ...
        """
       # target_expanded = tf.zeros([self.batch_size, self.max_sentence_len, self.embedding_dim]) + target
        
        context = tf.nn.dropout(context, keep_prob=keep_rate)
        context_left = tf.nn.dropout(context_left, keep_prob=keep_rate)
        context_right = tf.nn.dropout(context_right, keep_prob=keep_rate)
        with tf.name_scope('vanilla_attention_bilstm'):
            s = self.AT(context, target, sen_len, is_reuse=False) 
        with tf.name_scope('left_attention_bilstm'):
            s_l = self.AT(context_left, target, sen_len_left, is_reuse=True) 
        with tf.name_scope('right_attention_bilstm'):
            s_r = self.AT(context_right, target, sen_len_right, is_reuse=True) 
        
        z = tf.matmul(s, self.W1) + tf.matmul(target, self.U1) + self.b1  
        z_l = tf.matmul(s_l, self.W2) + tf.matmul(target, self.U2) + self.b2   
        z_r = tf.matmul(s_r, self.W3) + tf.matmul(target, self.U3) + self.b3 
        
        z_all = tf.concat([z,z_l,z_r], axis=1)
        z_all = tf.reshape(z_all, [-1,3, 2*self.n_hidden])
        gated = tf.nn.softmax(z_all, dim=1)
        
        #gated = tf.nn.softmax(tf.concat([z,z_l,z_r], axis=1), dim=1)
        #gated = tf.reshape(gated, [-1, 3, 1]) 
        #gated = tf.tile(gated, [1,1, 2*self.n_hidden])
        s_all = tf.concat([s, s_l, s_r], axis=1)
        s_all = tf.reshape(s_all, [-1, 3, 2*self.n_hidden])    
        outputs = tf.reduce_sum(tf.multiply(gated, s_all), axis=1)
        #outputs = tf.reduce_sum(tf.multiply(gated, s_all), axis=1) # batch_size X 2*n_hidden
        #outputs = tf.reduce_max(s_all, axis=1)  
    ...
    def forward(self, xa_inputs, hyper_inputs):
        with tf.name_scope('forward'):
            ...
            logits = self.bilstm_att_g(target, inputs_x, inputs_fw, inputs_bw, xa_inputs['sen_len'], xa_inputs['sen_len_fw'], xa_inputs['sen_len_bw'], hyper_inputs['keep_rate'])
        return logits
    ...
    def run(self, sess, train_data, test_data, n_iter, keep_rate, save_dir):
        ...
        with tf.name_scope('loss'):
            ...
            reg_losses = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
            cost = cost +  tf.reduce_sum(reg_losses)
        ...
        for i in range(n_iter):
            ...
            for samples, in train_data:
                ...
                for samples, in test_data:
                    ...
                    num = len(samples)
                    _logits, _loss, _acc, summary = sess.run([logits, cost, accuracy, test_summary_op], feed_dict=feed_dict)
                    acc += _acc
                    loss += _loss * num
                    cnt += num
                    y_pred.extend(list(np.argmax(_logits,1)))
                    y_true.extend(np.argmax(get_y(samples), 1))

                test_summary_writer.add_summary(summary, step)
                if acc / (1.0 * cnt) > 0.72: 
                    print('Iter {}: mini-batch loss={:.6f}, test acc={:.6f}, test f1={:.6f}'.format(step, loss / cnt, acc / cnt, f1_score(y_true, y_pred, average='macro')))
                test_summary_writer.add_summary(summary, step)
                if acc / cnt > max_acc:
                    max_acc = acc / cnt
...
def main(_):
    ...
    with tf.Session(config=configproto) as sess:
        ...
        model = BilstmAttGClassifier(word2idx=word2idx, 
...
if __name__ == '__main__':
    ...
    tf.app.flags.DEFINE_integer('n_iter', 1000, 'number of train iter')
    ...
    tf.app.flags.DEFINE_float('keep_rate', 0.7, 'keep rate')
