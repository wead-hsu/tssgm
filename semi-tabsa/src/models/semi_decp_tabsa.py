...
from src.models.decoder.ap_decoder import APDecoder
...
class SemiTABSA(BaseModel):
    def __init__(self, word2idx, embedding_dim, batch_size, n_hidden, learning_rate, n_class, max_sentence_len, l2_reg, embedding, dim_z, pri_prob_y, decoder_type, grad_clip, n_hidden_ae, position_enc, bidirection_enc, position_dec, bidirection_dec, classifier_type, sharefc):
        ...
        with tf.variable_scope('decoder'):
            self.decoder = APDecoder(word2idx=word2idx, 
                    embedding_dim=embedding_dim, 
                    n_hidden=n_hidden_ae, 
                    learning_rate=learning_rate, 
                    n_class=n_class, 
    ...
    def run(self, sess, train_data_l, train_data_u, test_data, n_iter, keep_rate, save_dir, batch_size, alpha, FLAGS):
        ...
        with tf.name_scope('labeled'):
            ...
            with tf.variable_scope('decoder'):
                ...
                decoder_loss_l = self.decoder.forward(self.decoder_xa_l, self.decoder_y_l, None, self.decoder_hyper_l)
            elbo_l = decoder_loss_l - pri_loss_l
        ...
        with tf.name_scope('unlabeled'):
            ...
            batch_size = tf.shape(list(self.decoder_xa_u.values())[0])[0]
            for idx in range(self.n_class):
                ...
                with tf.variable_scope('decoder', reuse=True):
                    ...
                    _decoder_loss = self.decoder.forward(self.decoder_xa_u, {'y':_label}, None, self.decoder_hyper_u)
                    print(_decoder_loss)
                    decoder_loss_u.append(_decoder_loss)

                _elbo_u = _decoder_loss# - _pri_loss_u
        ...
        batch_size_l = tf.shape(list(self.decoder_xa_l.values())[0])[0]
        ...
        test_summary_op = tf.summary.merge([summary_acc_test])

        logger = ExpLogger('semi_tabsa', save_dir)
        logger.write_args(FLAGS)
        logger.write_variables(tf.trainable_variables())
        logger.file_copy(['*.py', '*.sh', 'encoder/*.py', 'decoder/*.py', 'classifier/*.py'])
        ...
        for i in range(n_iter):
            ...
            for samples, in train_data_l:
                ...
                _, _acc, _loss, _step, summary = sess.run([optimizer, classifier_acc_l, decoder_loss_l, self.global_step, train_summary_op], feed_dict=feed_dict)
                summary_writer.add_summary(summary, _step)
                if np.random.rand() < 1/4:
                    print(_acc, _loss, _step)
            
            truth, pred, acc, loss, cnt = [], [], 0., 0., 0.
            ...
            for samples, in test_data:
                ...
                num, _pred, _acc, _loss, _step = sess.run([batch_size_l, pred_l, classifier_acc_l, decoder_loss_l, self.global_step], feed_dict=feed_dict)
            ...
            summary, _step = sess.run([test_summary_op, self.global_step], feed_dict={test_acc: acc/cnt})
            summary_writer.add_summary(summary, _step)
...
def main(_):
    ...
    save_dir = FLAGS.save_dir + '/' + str(timestamp) + '_AP' +  '_r' + str(FLAGS.learning_rate) + '_l2' + str(FLAGS.l2_reg)\
...
if __name__ == '__main__':
    ...
    tf.app.flags.DEFINE_string('bidirection_enc', 'False', 'boolean')
    tf.app.flags.DEFINE_string('position_dec', '', '[binary, distance, ], None for no position embedding')
    tf.app.flags.DEFINE_string('bidirection_dec', 'False', 'boolean')
