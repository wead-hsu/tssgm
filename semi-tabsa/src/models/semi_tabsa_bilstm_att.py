...
#            self.target_embedding = tf.get_variable('target_embedding', [len(target2idx), embedding_dim], initializer=tf.constant_initializer(target_embedding))
        ...
        with tf.variable_scope('classifier'):
            ...
            elif self.classifier_type == "BILSTM_ATT_G":
                 self.classifier = BilstmAttGClassifier(word2idx=word2idx, 
    ...
    def run(self, sess, train_data_l, train_data_u, test_data, n_iter, keep_rate, save_dir, batch_size, alpha, FLAGS):
        ...
        self.loss = tf.reduce_mean(self.loss_l  + self.loss_u + classifier_loss_l * alpha)
        ...
        with tf.name_scope('train'):
            ...
            optimizer = self.training_op(self.loss, tf.trainable_variables(), self.grad_clip, 20, self.learning_rate, grads=None, opt='Adam')
        ...
        for i in range(n_iter):
            ...
            #print(acc)
                ...
                logger.info('Iter {}: mini-batch loss={:.6f}, test acc={:.6f}, test f1={:.6f}'.format(_step, loss / cnt, acc / cnt, f1))
            #print(save_dir)
                if acc / cnt > max_acc:
                    max_acc = acc / cnt
                if f1 < max_f1:
...
def main(FLAGS=None):
   ...
   # val_it = BatchIterator(len(val), FLAGS.batch_size, [val], testing=False)
    ...
    with tf.Session(config=configproto) as sess:
        ...
        max_acc, max_f1 = model.run(sess, train_it, unlabel_it, test_it, FLAGS.n_iter, FLAGS.keep_rate, save_dir, FLAGS.batch_size, FLAGS.alpha, vars(FLAGS)['__flags'])
    return max_acc, max_f1
