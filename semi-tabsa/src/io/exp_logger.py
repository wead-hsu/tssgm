...
class ExpLogger(object):
    def __init__(self, lp='', ldir='../results', write_file=True):
        ...
        if write_file:
            ...
            self.logger.info('{} is created'.format(self.log_dir_path))
        else:
            self.logger = logging.getLogger()

    def write_args(self, args):
        self.info("-------- Parameter Info --------")
        sorted_args = sorted(args.items(), key=lambda x: x[0])
        for idx, item in enumerate(sorted_args):
            self.info("{}: {} = {}".format(str(idx), item[0], item[1]))
        self.info('--------------------')

    def info(self, str_line, write_file=True):
    ...
    def write_variables(self, param_list):
        self.info("-------- Model Variables --------")
        for idx, param in enumerate(param_list):
            self.info(str(idx) + '. ' + str(param.name) + ': ' + str(param.get_shape()))
        self.info('--------------------')
...
if __name__ == '__main__':
    explogger = ExpLogger('test', 'tmp')
    explogger.info('1')
    explogger.info('1', True)
    print('2')
